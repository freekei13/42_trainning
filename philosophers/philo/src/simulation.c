...
/*   Updated: 2026/02/28 21:59:06 by csamakka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*routine(void *philo_db)
{
	...
	while (1)
	{
		...
		if (p_db->db->someone_die > 0 || p_db->done_eat > 0)
		{
		...
		pthread_mutex_unlock(&p_db->db->db_mutex.p_print);
		pthread_mutex_lock(&p_db->philo_mutex.last_meal);
		p_db->time_left = p_db->db->time_to_die - ms_now(p_db->last_meal);
		pthread_mutex_unlock(&p_db->philo_mutex.last_meal);
		if (p_db->time_left > p_db->db->time_to_eat)
			usleep(p_db->time_left / 2 * 1000);
		else
			return (NULL);
	}
	return (NULL);
...
void	*reaper(void *philo_db)
{
	...
	while (1)
	{
		...
		if (p_db[i].done_eat == 0)
		{
			...
			if (p_db[i].meal_eaten >= p_db[i].db->must_eat)
			{
